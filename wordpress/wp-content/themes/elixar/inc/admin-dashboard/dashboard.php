...
if ( ! function_exists( 'elixar_admin_scripts' ) ) :
    ...
    function elixar_admin_scripts( $hook ) {
        $minified_assests = intval( get_theme_mod( 'elixar_minified_assests' ) ) == 1 ? true : false;
        $min = ''; 
        if ( $minified_assests == true ) {
            $min = '.min';
        }
        if ( $hook === 'widgets.php' || $hook === 'appearance_page_ft_elixar'  ) {
            wp_enqueue_style( 'elixar-admin-css', get_template_directory_uri() . '/inc/admin-dashboard/css/elixar-admin'.$min.'.css' );
            wp_enqueue_style( 'elixar-plugins', get_template_directory_uri() . '/inc/admin-dashboard/css/recommend.css' );
